...
class ConvNet(object):
    ...
    def inference(self):
        conv1 = tf.layers.conv2d(inputs=self.img, # channels last : NHWC
                                  filters=32, # number of output dimensions
                                  kernel_size=[5, 5],
                                  padding='SAME',
        ...
        pool2 = tf.reshape(pool2, [-1, feature_dim]) # stretch out to 1-d tensor
    ...
    def build(self):
        ...
        '''
        self.get_data() # get data init op
        self.inference()
        self.loss()
        self.optimize()
        self.eval()
    ...
    def train(self, n_epochs):
        ...
        train_writer = tf.summary.FileWriter('./graphs/convnet_layers/train', tf.get_default_graph())
        test_writer = tf.summary.FileWriter('./graphs/convnet_layers/eval', tf.get_default_graph())
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(n_epochs):
                step = self.train_one_epoch(sess, saver, self.train_init, train_writer, epoch, step)
    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            if ckpt and ckpt.model_checkpoint_path:
                saver.restore(sess, ckpt.model_checkpoint_path)
            step = self.gstep.eval()
            self.eval_once(sess, self.test_init, writer, step, step)
        writer.close()
    ...
    def check(self, n_epochs):
        ...
        writer = tf.summary.FileWriter('./graphs/initcheck', tf.get_default_graph())
